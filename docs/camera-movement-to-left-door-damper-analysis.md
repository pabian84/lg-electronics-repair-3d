# 왼쪽 도어 댐퍼 카메라 이동 동작 분석

## 개요
`AnimatorAgent.ts`에서 `await this.cameraMovementService.moveCameraToLeftDoorDamper();` 함수 호출 시, 왼쪽 도어 댐퍼 위치로 카메라를 이동시키는 방식과 동작 순서를 분석한 문서입니다.
...
### 2.2 문 열기 애니메이션 실행 (`AnimatorAgent.ts` line 905-931)
...
### 2.3 애니메이션 완료 대기 (`AnimatorAgent.ts` line 937-943)
...
### 2.4 카메라 이동 호출 (`AnimatorAgent.ts` line 946-964)
...
### 3.1 `moveCameraToLeftDoorDamper()` 메서드 (line 39-49)
...
### 3.2 `moveCameraCinematic()` 메서드 상세 분석 (line 55-244)

#### 3.2.1 초기화 단계 (line 56-73)
...
#### 3.2.4 Damping 비활성화 (line 124-131)
...
#### 3.2.5 GSAP 애니메이션 실행 (line 133-243)
...
#### 3.2.6 노드 하이라이트 적용 (line 206-238)
...
- [카메라 이동 고려사항](../.kilocode/rules/README_2601231159.md)
- [GSAP를 사용한 복잡한 이징 효과 및 카메라이동 구현](../.kilocode/rules/README_2601231159.md)
- [3D 모델 좌표 추출 고려사항](../.kilocode/rules/3d-model-coordinate-extraction-considerations.md)
